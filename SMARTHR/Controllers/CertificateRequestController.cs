...
namespace SMARTHR.WEB.Controllers
{
    public class CertificateRequestController : Controller
    {
        ...
        public async Task<CertificateResponse?> DeleteAttachedCertificateRequest(string CertificateId)
        {
            string? companyid = HttpContext.Session.GetString("CompanyCode");
            string? userid = HttpContext.Session.GetString("Username");
            ViewBag.Username = userid;
            ...
            try
            {
                ...
                var apiUrl = $"{_configuration["D365:BaseUrl"]}/FCC_ESSIntgerationServiceGroup/FCC_ESSAttachmentService/deleteAttachFile";

                var requestBody = new DeleteAttachedCretificateResponseRequest
                {
                    deleteAttachFile = new DeleteAttchedContractRequestDetails
                    {
                        companyId = companyid,
                        attachRecID = CertificateId,
