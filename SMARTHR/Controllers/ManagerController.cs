	...
	public class ManagerController : Controller
	{
		...
		public async Task<IActionResult> ReadManagerLevelRequest(string leaveId)
		{
			var unifiedResponse = new UnifiedLeaveResponse
			{
                ...
                ITAssets = new List<ITAssetsModels>(),
                LeaveDetails = new List<LeaveDetails>(),
				Success = false,
				loanList =new List<LoanItems>(),
				loanRequestResult = new List<LoanRequestVM>() 
			};
			...
			try
			{
				...
				else if (requestType?.Trim() == TranTypes.ITAssets.Trim())
				{
					apiUrl = $"{_configuration["D365:BaseUrl"]}{D365ApiURL.ReadAssetRequestUrl}";
					...
					if (response.IsSuccessStatusCode)
					{
						try
						{
							var itAssets = JsonSerializer.Deserialize<ITAssetsReadInfoResponse>(responseContent, jsonOptions);
							if (itAssets == null || !itAssets.assetRequestList.Any())
							{
								_logger.LogWarning("No business trip details returned for BusinessTripReqId: {BusinessTripReqId}", leaveId);
								unifiedResponse.Message = "No business trip details found.";
								return Ok(unifiedResponse);
							...
							unifiedResponse.ITAssets = itAssets.assetRequestList;
							unifiedResponse.Success = true;

							if (itAssets.assetRequestList.Any() && itAssets.assetRequestList[0].attachRefList != null)
							{
								var attachmentsJson = JsonSerializer.Serialize(itAssets.assetRequestList[0].attachRefList);
								HttpContext.Session.SetString("attachments", attachmentsJson);
								ViewBag.Attachments = JsonSerializer.Deserialize<List<AttachRefItem>>(attachmentsJson);
							}
						}
				...
				else if (requestType?.Trim() == TranTypes.LoanRequest.Trim())
				{
					apiUrl = $"{_configuration["D365:BaseUrl"]}{D365ApiURL.ReadLoanRequestUrl}";
					...
					if (response.IsSuccessStatusCode)
					{
						try
						{
							var loanResult = JsonSerializer.Deserialize<LoanRequestResult>(responseContent, jsonOptions);
							if (loanResult == null || !loanResult.loanRequestResult.Any())
							{
								_logger.LogWarning("No business trip details returned for BusinessTripReqId: {BusinessTripReqId}", leaveId);
								unifiedResponse.Message = "No business trip details found.";
								return Ok(unifiedResponse);
							...
							unifiedResponse.loanRequestResult = loanResult.loanRequestResult;
							unifiedResponse.Success = true;

							if (loanResult.loanRequestResult.Any() && loanResult.loanRequestResult[0].attachRefList != null)
							{
								var attachmentsJson = JsonSerializer.Serialize(loanResult.loanRequestResult[0].attachRefList);
								HttpContext.Session.SetString("attachments", attachmentsJson);
								ViewBag.Attachments = JsonSerializer.Deserialize<List<AttachRefItem>>(attachmentsJson);
							}
						}
		...
		public async Task<IActionResult> ReadManagerLeaveResumption(string leaveId)
		{
			...
			try
			{
                ...
                else if (requestType?.Trim() == TranTypes.Certificate.Trim())
                {
                    apiUrl = $"{_configuration["D365:BaseUrl"]}" + D365ApiURL.ReadCertificateUrl;
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        try
                        {
                            ...
                            if (employeeResponse == null || employeeResponse.CertificateDetails == null || !employeeResponse.CertificateDetails.Any())
                            {
                                _logger.LogWarning("No leave details returned for LeaveId: {LeaveId}", leaveId);
                                unifiedResponse.Message = "No leave details found.";
                                return Ok(unifiedResponse);
                            ...
                            unifiedResponse.CertificateDetails = employeeResponse.CertificateDetails; // Assuming types match
                            unifiedResponse.Success = true;
                            unifiedResponse.loanList = ViewBag.LoanUser as List<LoanItems>;

                            if (employeeResponse.CertificateDetails.Any() && employeeResponse.CertificateDetails[0].AttachRefList != null)
                            {
                                var attachmentsJson = JsonSerializer.Serialize(employeeResponse.CertificateDetails[0].AttachRefList);
                                HttpContext.Session.SetString("attachments", attachmentsJson);
                                ViewBag.Attachments = JsonSerializer.Deserialize<List<AttachRefItem>>(attachmentsJson);
                            }
                        }
