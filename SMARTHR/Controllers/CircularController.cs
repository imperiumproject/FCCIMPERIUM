...
namespace SMARTHR.WEB.Controllers
{
    public class CircularController : Controller
    {
        ...
        public async Task<IActionResult> Circular(circularList input,documentContract res)
        {
        ...
        {
             var res = await ReadDocumentDownload(id);

            var model = res?.DocumentDownloadDetials ?? new List<SMARTHR.WEB.Models.DocumentDownloadDetail>();
            return View(model);
        }

        [HttpPost]
        public async Task<DocumentDownloadResponse> ReadDocumentDownload(string Empid)
        {
            var document = new DocumentDownloadResponse();
            string? companyId = HttpContext.Session.GetString("CompanyCode");
            string? username = HttpContext.Session.GetString("Username");
            try
            {
                ...
                var apiUrl = $"{_configuration["D365:BaseUrl"]}" + D365ApiURL.ReadDocumentList;


                var requestBody = new documentContractRequest
                {
                ...
                if (response.IsSuccessStatusCode)
                {
                    var fileResponse = JsonSerializer.Deserialize<DocumentDownloadResponse>(responseContent, jsonOptions);
                    document = fileResponse ?? new DocumentDownloadResponse();
                }
                else
                {
                    _logger.LogError("API call failed: {StatusCode}, {ReasonPhrase}", response.StatusCode, response.ReasonPhrase);
